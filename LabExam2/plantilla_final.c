...
dup2(fd, fd_substituit);            // Sobreescriu fd a fd_substituit (mirar "Conectar pipe amb terminal o altres pipes" més avall)

dup(fd);      // Ocupa el primer canal lliure amb una copia de fd
...
////////////////////////////////////////////
    int *pids;
    pids = sbrk([*]); //retorna un punter a la memòria heap

    //[*]:
        // num > 0: augmenta el tamany del heap amb num bytes (ampliar)
        // num < 0: disminueix el tamany del heap amb num bytes (alliberar)
        // num == 0: el tamany es queda igual

    pids = malloc(tamany); //retorna punter a l'inici de memòria reservat
    //{...}
    free(pids); //alliberem la memòria
