...
alarm(n > 0); // --> En n s'enviarè un SIGALRM al procés
...
int ret;
while((ret = waitpid (-1, NULL, 0)) > 0){} // waitpid (-1, NULL, 0) == wait()
waitpid(pid, NULL, 0); //Esperar un fill en concret
while(waitpid (-1, NULL, WNOHANG) > 0){} // pels procesos zombies

/* Esperar fills */
while(waitpid (-1, NULL, 0) > 0) // waitpid (-1, NULL, 0) == wait()
//						[1]  [2]  [3]
// [1] -> Normalment -1 per qualsevol fill, o el pid del fill en concret
...
{...error...}                                // WEXITSTATUS(res) returna el valor d'exit (exit(valor))
                                             // https://www.ibm.com/docs/en/zos/2.4.0?topic=functions-waitpid-wait-specific-child-process-end
																						 // Realment no caldria WIFEXITED(res) ja que WEXITSTATUS(res) comprova fa ús d'aquesta internament
...
// ____________________ Què s'hereda quan? __________________________ //
...
/* _____________________ Documentació varia _______________________//

- Documentació comandes 		by @dumitrux	(https://github.com/dumitrux/SO-FIB/blob/master/Comandos.txt)
- C for dummies 						by @MrRobb 		(https://github.com/MrRobb/SO-FIB/blob/master/Utilidades/C%20para%20dummies.md)
- Comandes comunes 					by @MrRobb 		(https://github.com/MrRobb/SO-FIB/blob/master/comandos/comandos.pdf)
- Comandes linux comuns 		by @MrRobb 		(https://github.com/MrRobb/SO-FIB/tree/master/comandos/common_pdf)
- Template per signal 			by @MrRobb 		(https://github.com/MrRobb/SO-FIB/blob/master/Utilidades/signals_template.c)
- Template Usage i Errors 	by @MrRobb 		(https://github.com/MrRobb/SO-FIB/blob/master/Utilidades/UsageYErrores.c)
- Resum T1, T2 i T3 				by @MrRobb 		(https://github.com/MrRobb/SO-FIB/blob/master/Resumen/resum.md)
___________________________________________________________________
_______________________Exemple Makefile____________________________

all: program_llib_extra program_normal

program_llib_extra: sub_main.o sub_llib.o
	gcc -o program_llib_extra.x sub_main.o sub_llib.o

sub_llib.o: sub_llib.c
	gcc -c sub_llib.c

sub_main.o:
	gcc -c sub_main.c

program_normal: program_normal.c
	gcc -o program_normal.x program_normal.c

clean:
	rm *.o program_normal.x program_llib_extra.x

// .x -> executables; .o -> objecte; .c -> codi
// Per executar #make o #make -f NomFitxerMakefile
